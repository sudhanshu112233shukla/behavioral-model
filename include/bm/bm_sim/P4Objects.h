...
class P4Objects {
 ...
 private:
  ...
  void add_table_apply(const std::string &name, std::unique_ptr<TableApply> table_apply);

  TableApply *get_table_apply_cfg(const std::string &name) const;
 ...
 private:
  ...
  // table applies
  std::unordered_map<std::string, std::unique_ptr<TableApply>> table_applies_map{};
