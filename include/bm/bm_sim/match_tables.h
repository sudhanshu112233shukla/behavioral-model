...
/**
 * Class representing a single application of a table in a pipeline.
 * This allows the same table to be applied multiple times in a pipeline
 * with different next nodes for each application.
 */
class TableApply : public ControlFlowNode {
 public:
  ...
  MatchTableAbstract *get_table() const { return table; }
...
class MatchTableAbstract : public NamedP4Object {
 ...
 public:
  ...
  // Apply the table with a specific table apply context
  const ControlFlowNode *apply_action(Packet *pkt, const TableApply *table_apply);
  ...
  // Lookup with a specific table apply context
  virtual const ActionEntry &lookup(const Packet &pkt, bool *hit,
                                    entry_handle_t *handle,
                                    const ControlFlowNode **next_node,
                                    const TableApply *table_apply) = 0;
  ...
  // Set which TableApply is using the cached next node values
  void set_cached_table_apply(const TableApply *table_apply) {
    cached_table_apply = table_apply;
  }
 ...
 protected:
  ...
  bool has_next_node_hit{false};
  bool has_next_node_miss{false};
  // The TableApply that is using the cached next node values
  const TableApply *cached_table_apply{nullptr};
...
class MatchTable : public MatchTableAbstract {
 ...
 public:
  ...
  const ActionEntry &lookup(const Packet &pkt, bool *hit,
                            entry_handle_t *handle,
                            const ControlFlowNode **next_node,
                            const TableApply *table_apply) override;
