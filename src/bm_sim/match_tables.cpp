...
TableApply::operator()(Packet *pkt) const {
  return table->apply_action(pkt, this);
}
...
MatchTableAbstract::apply_action(Packet *pkt) {
  return apply_action(pkt, cached_table_apply);
}

const ControlFlowNode *
MatchTableAbstract::apply_action(Packet *pkt, const TableApply *table_apply) {
  ...
  const ActionEntry &action_entry = lookup(*pkt, &hit, &handle, &next_node, table_apply);
...
MatchTable::lookup(const Packet &pkt, bool *hit, entry_handle_t *handle,
                   const ControlFlowNode **next_node) {
  return lookup(pkt, hit, handle, next_node, cached_table_apply);
}

const ActionEntry &
MatchTable::lookup(const Packet &pkt, bool *hit, entry_handle_t *handle,
                   const ControlFlowNode **next_node,
                   const TableApply *table_apply) {
  MatchUnitAbstract<ActionEntry>::MatchUnitLookup res = match_unit->lookup(pkt);
  *hit = res.found();
  *handle = res.handle;
  const auto &entry = (*hit) ? (*res.value) : default_entry;

  // If we're using a specific table apply context and it's not the cached one,
  // we need to determine the next node at runtime
  if (table_apply && table_apply != cached_table_apply) {
    if (*hit) {
      if (table_apply->has_next_node_hit) {
        *next_node = table_apply->next_node_hit;
      } else {
        *next_node = table_apply->next_nodes.at(entry.action_fn.get_action_id());
      }
    } else {
      if (table_apply->has_next_node_miss) {
        *next_node = table_apply->next_node_miss;
      } else {
        *next_node = table_apply->next_nodes.at(entry.action_fn.get_action_id());
      }
    }
  } else {
    // Use the cached next node from the entry
  ...
  if (!(*hit) && !default_set) {
    if (table_apply && table_apply != cached_table_apply) {
      if (table_apply->has_next_node_miss) {
        ...
        *next_node = table_apply->next_nodes.at(default_default_entry.action_fn.get_action_id());
      }
    } else {
...
MatchTableIndirectWS::lookup(const Packet &pkt, bool *hit,
                             ...
                             const ControlFlowNode **next_node,
                             const TableApply *table_apply) {
  return MatchTableIndirect::lookup(pkt, hit, handle, next_node, table_apply);
}
