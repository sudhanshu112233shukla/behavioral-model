...
constexpr int max_minor_version = 25; // Increased to support table_applies
// not needed for now
...
bool json_version_supports_table_applies(const Json::Value &cfg_root) {
  if (!cfg_root.isMember("__meta__")) return false;
  const auto &cfg_meta = cfg_root["__meta__"];
  if (!cfg_meta.isMember("version")) return false;
  const auto &cfg_version = cfg_meta["version"];
  if (cfg_version.size() != 2) return false;
  auto major = cfg_version[0].asInt();
  auto minor = cfg_version[1].asInt();
  return (major == required_major_version && minor >= 25);
}
...
P4Objects::init_pipelines(const Json::Value &cfg_root,
  ...
  for (const auto &cfg_pipeline : cfg_pipelines) {
    ...
    // Check if the JSON version supports table_applies
    bool supports_table_applies = json_version_supports_table_applies(cfg_root);

    // Create table_applies if supported
    if (supports_table_applies && cfg_pipeline.isMember("table_applies")) {
      const auto &cfg_table_applies = cfg_pipeline["table_applies"];
      DupIdChecker dup_id_checker_table_apply("table_apply");
      ...
      for (const auto &cfg_table_apply : cfg_table_applies) {
        ...
        dup_id_checker_table_apply.add(table_apply_id);

        const string table_name = cfg_table_apply["table"].asString();
        MatchTableAbstract *table = get_abstract_match_table(table_name);

        auto table_apply = std::unique_ptr<TableApply>(
            new TableApply(table_apply_name, table_apply_id, table));

        const Json::Value &cfg_next_nodes = cfg_table_apply["next_tables"];
        ...
        std::string actions_key = "actions";
        if (cfg_table_apply.isMember("action_ids")) {
          actions_key = "action_ids";
        }

        const Json::Value &cfg_actions = cfg_table_apply[actions_key];
        bool next_is_hit_miss = false;
        check_next_nodes(cfg_next_nodes, cfg_actions, table_apply_name,
                         &next_is_hit_miss);
        ...
        for (const auto &cfg_action : cfg_actions) {
          ...
          if (!next_is_hit_miss && !cfg_next_nodes.isMember(action_name)) {
            throw json_exception(
                EFormat() << "Table apply '" << table_apply_name << "' should have key"
        ...
        if (next_is_hit_miss) {
          table_apply->set_next_node_hit(get_next_node(cfg_next_nodes["__HIT__"]));
          table_apply->set_next_node_miss(get_next_node(cfg_next_nodes["__MISS__"]));
        }

        if (cfg_table_apply.isMember("base_default_next")) {
          table_apply->set_next_node_miss_default(
              get_next_node(cfg_table_apply["base_default_next"]));
        }

        // If this is the first apply of this table, set it as the cached apply
        if (table->cached_table_apply == nullptr) {
          table->set_cached_table_apply(table_apply.get());
        }

        add_table_apply(table_apply_name, std::move(table_apply));
      }
    }

    // next node resolution for tables (backward compatibility)
    if (!supports_table_applies || !cfg_pipeline.isMember("table_applies")) {
      for (const auto &cfg_table : cfg_tables) {
        const string table_name = cfg_table["name"].asString();
        MatchTableAbstract *table = get_abstract_match_table(table_name);
...
void
P4Objects::add_table_apply(const std::string &name, std::unique_ptr<TableApply> table_apply) {
  add_control_node(name, table_apply.get());
  add_new_object(&table_applies_map, "table apply", name, std::move(table_apply));
}

TableApply *
P4Objects::get_table_apply_cfg(const std::string &name) const {
  return get_object(table_applies_map, "table apply", name).get();
}
